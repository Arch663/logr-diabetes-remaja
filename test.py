...
# calculate logit scores and probabilities
logit_scores = model.decision_function(X_test)  # Calculate logit scores on the test set
probabilities = model.predict_proba(X_test)[:, 1]  # Calculate probabilities on the test set

# Sort the logit scores and probabilities
sorted_indices = np.argsort(logit_scores)
sorted_logits = logit_scores[sorted_indices]
...
<div class="hero">
  <h1>Prediksi Diabetes Melitus Untuk Anak Usia Remaja<br>Dengan Logistic Regression</br></h1>
</div>
...
with tab1:
    ...
    if 'input_data' not in st.session_state:
        st.session_state.input_data = pd.DataFrame(columns=["Usia", "Jenis_Kelamin", "BMI", "Kadar_Gula_Darah", "Tekanan_Darah", "HbA1c", "Hasil_Tes"])
    #form
    ...
    with st.form("diabetes_form"):
        ...
        if submitted:
            ...
            probability = model.predict_proba(input_values_scaled)
            ...
            st.write("Anda Memiliki", str(round(probability_scalar * 100)) + "%", "Kemungkinan Terkena diabetes mellitus.")
            if hasil_tes == 1:
...
with tab2:
    ...
    with col2:
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=sorted_logits, y=sorted_probs, mode='markers', 
                                marker=dict(color='#0E8388', opacity=0.5), name='Data'))
        fig.add_shape(type='line', x0=min(sorted_logits), y0=0.5, 
                    x1=max(sorted_logits), y1=0.5, 
                    line=dict(color='grey', dash='dash'), name='Threshold 0.5')
        fig.update_layout(xaxis_title='Logit (z) Score', yaxis_title='Predicted Probability', showlegend=True)
        st.plotly_chart(fig)
